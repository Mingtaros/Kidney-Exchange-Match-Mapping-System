...
algorithm will be fitted with the same dataset group 50 times for every group. As seen in Figure 3, each data point in the boxplot
...
As seen in Figure 4, there are two plots, each one representing a group of fast algorithms and a group of slow algorithms.
...
overall line plot, we can conclude that the relation of data size and execution time is a form of polynomial function, not linear.
...
In this experiment, each combination is used for every data seed. To ease up the comparison, in Figure 5, we use ratio of improvement
...
\begin{figure*}
    \includegraphics[width=\textwidth]{images/matching_efficiency_ratio_against_baseline.png}
    \caption{Matching Efficiency Ratio of \textit{N}-way Algorithms against Baseline}
\end{figure*}

To simplify the figure, Table II is used to store average matching efficiency improvement ratio in compared to baseline. The average used
...
\begin{table}[htbp]
    ...
    \hline
    &\textbf{Average} \\
    \textbf{Algorithm}&\textbf{Improvement}\\
    &\textbf{Ratio}\\
    \hline
...
As we can see from Table II, most of \textit{N}-way match map searching algorithm produces higher matching efficiency in
